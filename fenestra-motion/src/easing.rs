//! Per-segment easing: linear, hold, CSS cubic-bezier curves (shared with the
//! interactive engine via [`CubicBezier`]), and the closed-form damped spring
//! (shared via [`SpringSpec::step`](fenestra_core::SpringSpec)).

use fenestra_core::{CubicBezier, SpringSpec};
...
pub enum Ease {
    ...
    /// outside `0..=1` overshoot: scalar tracks extrapolate past their
    /// endpoints mid-segment, color tracks clamp at the target.
    Bezier(CubicBezier),
...
/// envelope below 0.1% — the tolerance shared with the interactive engine).
/// A segment shorter than the settle time truncates the tail: the next key
/// still lands exactly.
#[derive(Debug, Clone, Copy, PartialEq)]
pub struct Spring {
...
/// [`Spring::velocity`] via struct update or use [`Spring`] directly for a
/// launched start.
...
/// settles, `cubic-bezier(0.34, 1.56, 0.64, 1)`. Scalars extrapolate past
/// their endpoint mid-segment; colors clamp.
