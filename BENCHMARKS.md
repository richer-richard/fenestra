...
on your machine; numbers below are one snapshot (refreshed at 0.10.0), not a promise.
...
| Scene | mean | best |
| --- | ---: | ---: |
| counter (tiny) | 0.011 ms | 0.010 ms |
| gallery_controls (medium, ~hundreds of nodes) | 0.195 ms | 0.188 ms |
| gallery_display (large, tables + icons) | 0.305 ms | 0.294 ms |
...
| Scene | mean | best |
| --- | ---: | ---: |
| gallery_display @ 760×1190 | 2.533 ms | 2.372 ms |
| counter @ 320×160 | 1.729 ms | 1.552 ms |
...
- Full rebuild means per-frame cost scales with *visible* tree size.
  Virtualize long lists (`virtual_list`, or `virtual_list_variable` for
  mixed-height rows); a 10k-node static tree would be felt.
- The windowed runner memoizes *clean* frames (re-presents the cached
  scene when nothing changed), so idle apps cost ~zero CPU — but these
  numbers measure the rebuild path, which is what animation and
  interaction pay.
